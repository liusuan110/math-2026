...
detection reaches 0.991 mAP50 and 0.786 mAP50-95, while pin pose estimation
reaches 0.947 and 0.829. INT8 execution on an Intel Core Ultra 5 225U neural processing
...
circuit board inspection, or netlists that are already
symbolic~\cite{amsnet}. Simulation
...
Accepted pin-hole relations are grouped into electrical nets using the known
breadboard layout, in which each five-hole strip is internally connected and the
supply rails span the board. A disjoint-set structure merges the strips joined by
detected jumper wires, and the result is a component--net graph: nets are nodes,
a two-terminal component is an edge between the two nets it joins, and a package
that spans more than two nets keeps a node of its own with numbered pins.
Physical holes remain the only source of net identity, so a corrected hole
assignment regenerates the labels instead of inheriting stale ones.
...
runtime~\cite{intel225u,openvino}. Between 60 and 1153 sequential inferences
...
\caption{Topology readout on a second board, produced by one run of the full
pipeline. (a) Every detected terminal is bound to a hole and colored by the net
it joins; thin links connect the terminals of one net, and dashed red rings mark
the six terminals the mapper itself flagged as ambiguous. (b) The resulting
component--net graph, drawn from the exported netlist with the same net colors:
nets are nodes, a two-terminal component is an edge between the two nets it
joins, and the DIP-8 package keeps a node of its own with numbered pins because
it spans more than two nets. Twelve components and 30 terminals resolve into ten
nets; the five jumper wires contribute no edges but merge strips, and the nets
they created are tagged with the wires responsible. Pins 1, 5 and 8 of the
package reach single-terminal nodes and are reported as unconnected.}
\label{fig:netlist}
\end{figure*}
