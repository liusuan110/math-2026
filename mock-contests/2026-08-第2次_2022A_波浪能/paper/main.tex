...
针对问题四，将垂荡与纵摇子系统的频域周期稳态平均功率严格分解为两个单变量函数，并利用 Sherman--Morrison 秩一更新推导解析最优条件。得到直线阻尼最优系数为 $59152.916113\ \mathrm{N\,s/m}$，旋转阻尼最优系数为上边界 $100000\ \mathrm{N\,m\,s}$，最大总平均功率为 $318.679256\ \mathrm{W}$。全域扫描、二维差分进化、可行邻域扰动和周期时域能量积分均验证了该约束全局最优解。
...
\end{equation}
应用 Sherman--Morrison 秩一更新公式，得到相对位移复振幅和平均功率
\begin{equation}
...
直线阻尼的无约束最优点为 $59152.916113\ \mathrm{N\,s/m}$，位于区间内部；旋转阻尼的无约束最优点为 $120114.390453\ \mathrm{N\,m\,s}$，超过题设上限。因此旋转阻尼在可行区间内随系数增大而增功，$c_\theta=100000$ 为活跃上边界。
...
\begin{table}[H]
  ...
  \begin{tabular}{lc}
    ...
    指标 & 最优值 \\
    \midrule
    直线阻尼系数 $c_z^*$ & $59152.916113\ \mathrm{N\,s/m}$ \\
    旋转阻尼系数 $c_\theta^*$ & $100000\ \mathrm{N\,m\,s}$ \\
    直线 PTO 平均功率 & $318.336440\ \mathrm{W}$ \\
    旋转 PTO 平均功率 & $0.342817\ \mathrm{W}$ \\
    最大总平均功率 & $318.679256\ \mathrm{W}$ \\
    单周期总输出能量 & $1010.966789\ \mathrm{J}$ \\
    \bottomrule
  \end{tabular}
\end{table}

旋转 PTO 仅占总平均功率的 $0.107574\%$，表明情形 4 中装置输出主要由垂荡相对运动贡献。为排除解析分解和单峰判断带来的路径依赖，本文对两个阻尼区间分别进行 $10001$ 点全域扫描和有界连续优化，并在完整二维参数域内独立执行差分进化搜索。二维搜索经 $1554$ 次目标函数评价得到近似点 $(59152.895070,99999.999943)$，总功率为 $318.679256467\ \mathrm{W}$，与解析约束最优值一致。图 \ref{fig:q4-surface} 以对数色标给出参数域内相对最优点的功率损失 $\overline P_{\max}-\overline P$，图中色值并非总功率本身。
...
在垂荡与纵摇联合优化中，本文利用一阶模型的块对角结构将二维总功率目标严格分解，并以秩一更新得到解析最优条件。最优直线阻尼为 $59152.916113\ \mathrm{N\,s/m}$，最优旋转阻尼取上边界 $100000\ \mathrm{N\,m\,s}$，最大总平均功率为 $318.679256\ \mathrm{W}$。解析推导、穷举扫描、二维差分进化与周期时域能量闭合相互一致，使最终结果同时具备全局最优性、数值可复现性和物理自洽性。
