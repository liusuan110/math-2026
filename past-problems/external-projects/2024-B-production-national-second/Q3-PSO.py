...
def fitness(particle: Particle, row: pd.Series, num_simulations: int) -> float:
    ...
    component_params = [
        [particle.position[i], row[f'c{i + 1}_defect_rate'], row[f'c{i + 1}_purchase_price'],
         row[f'c{i + 1}_inspection_cost']]
        for i in range(8)
    ]

    half_params = [
        [particle.position[i + 8], row[f'h{i + 1}_defect_rate'], row[f'h{i + 1}_assembly_cost'],
...
def plot_fitness_history(history: List[float], case_num: int):
    ...
    plt.grid(True)
    plt.savefig(f'./fig/Q3/fitness_history_PSO_Case_{case_num}.svg')
    plt.close()
...
output.to_excel('./data/Q3/Q3_PSO_output.xlsx', index=False)