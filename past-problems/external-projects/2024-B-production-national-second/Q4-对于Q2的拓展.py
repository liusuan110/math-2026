...
def plot_revenue_distirbute(case_df, case_num):
    ...
    plt.tight_layout()
    plt.savefig(f'./fig/Q4/Case_{case_num}_profit_distribution.svg')
    plt.close()
        ...
        for inspect_comp2 in states:
            for inspect_product in states:
                for disassemble in states:
                    c1_params = [inspect_comp1, convert_p_sampling_to_p_real(row['c1_defect_rate']), row['c1_purchase_price'], row['c1_inspection_cost']]
                    c2_params = [inspect_comp2, convert_p_sampling_to_p_real(row['c2_defect_rate']), row['c2_purchase_price'], row['c2_inspection_cost']]
                    p_params = [inspect_product, disassemble, convert_p_sampling_to_p_real(row['p_defect_rate']), row['p_assembly_cost'], row['p_inspection_cost'], row['p_market_price'], row['p_replacement_loss'], row['p_disassembly_cost']]
                    print(c1_params)
                    print(c2_params)
                    print(p_params)
...
output.to_excel('./data/Q4/Q4_output_Q2update.xlsx', index=False)