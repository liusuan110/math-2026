...
for year in range(2024, 2031):
    # 得到没有种植豆类年数
    year_ans = pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_2.csv", encoding='gbk')
    last_year_ans = pd.read_csv(f"preprocessed_data/整理数据/Q1/{year-1}/{year-1}_答案表_2.csv", encoding='gbk')
    ...
    year_ans.to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_2.csv", encoding='gbk', index=False)
    max_profit=0
    pass_num=0
    ...
    def objective(x):
        ...
        x_diff_1=x_sum_1-xiaoliang[0,:]
        x_diff_2=x_sum_2-xiaoliang[1,:]
        pd.set_option('display.max_rows', None)
        x_diff_1=np.maximum(x_diff_1,0)
        x_diff_2=np.maximum(x_diff_2,0)
        xiaoshou_jine = np.sum(x_min_1 * danjia[0, :]) + np.sum(x_min_2 * danjia[1, :])+0.5*np.sum(x_diff_1* danjia[0, :])+0.5*np.sum(x_diff_2* danjia[1, :])
        # 计算成本
    ...
    for i in range(max_iter):
        ...
        if(evaluate(x)):
            ...
            if new_profit>max_profit:
                ...
                pd.DataFrame(x).to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_解_2.csv",encoding='gbk',index=False)
                max_profit=new_profit


    with open(f"preprocessed_data/整理数据/Q1/{year}/{year}_利润_2.txt", 'w', encoding='utf-8') as file:
        file.write(str(max_profit))
        file.write('\n')
        file.write(str(pass_num))
    jie=pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_解_2.csv",encoding='gbk')
    ans=pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_2.csv",encoding='gbk')
    # 打印形状以进行调试
    ans.iloc[:, 1:42] = jie.values

    ans.to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_2.csv",encoding='gbk',index=False)
