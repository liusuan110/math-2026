...
for year in range(2024, 2031):
    # 得到没有种植豆类年数
    year_ans = pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年答案表.csv", encoding='gbk')
    last_year_ans = pd.read_csv(f"preprocessed_data/整理数据/Q2/{year-1}/{year-1}年答案表.csv", encoding='gbk')
    ...
    year_ans.to_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年答案表.csv", encoding='gbk', index=False)
    max_profit=0
    pass_num=0
    ...
    def objective(x):
        ...
        xiaoliang = pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年_销量表.csv", encoding='gbk').iloc[:,
                    1:].values
        danjia = pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年_单价表.csv", encoding='gbk').values
        chengben = pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年_成本表.csv", encoding='gbk').iloc[:,
                   1:42].values
        chanliang=pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年_产量表.csv", encoding='gbk').iloc[:,1:42].values
    ...
    for i in range(max_iter):
        ...
        if(evaluate(x)):
            ...
            if new_profit>max_profit:
                ...
                pd.DataFrame(x).to_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年解.csv",encoding='gbk',index=False)
                max_profit=new_profit


    with open(f"preprocessed_data/整理数据/Q2/{year}/{year}年利润.txt", 'w', encoding='utf-8') as file:
        file.write(str(max_profit))
        file.write('\n')
        file.write(str(pass_num))
    jie=pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年解.csv",encoding='gbk')
    ans=pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年答案表.csv",encoding='gbk')
    # 打印形状以进行调试
    ans.iloc[:, 1:42] = jie.values
    ans.to_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年答案表.csv",encoding='gbk',index=False)
