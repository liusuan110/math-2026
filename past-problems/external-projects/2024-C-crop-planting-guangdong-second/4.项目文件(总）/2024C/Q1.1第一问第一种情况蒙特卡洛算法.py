...
for year in range(2024, 2031):
    # 得到没有种植豆类年数
    year_ans = pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_1.csv", encoding='gbk')
    last_year_ans = pd.read_csv(f"preprocessed_data/整理数据/Q1/{year-1}/{year-1}_答案表_1.csv", encoding='gbk')
    ...
    for index, row in year_ans.iterrows():
        ...
        if not last_year_row.empty:
            ...
            # 获取 [1:42] 列范围内不在 bean_index_list 中的列索引
    ...
    year_ans.to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_1.csv", encoding='gbk', index=False)
    max_profit=0
    pass_num=0
    print(year_ans.iloc[50,44])
    ...
    def objective(x):
        ...
        xiaoliang = pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_销量表.csv", encoding='gbk').iloc[:,
                    1:].values
        danjia = pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_单价表.csv", encoding='gbk').values
        chengben = pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_成本表.csv", encoding='gbk').iloc[:,
                   1:42].values
        chanliang=pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_产量表.csv", encoding='gbk').iloc[:,1:42].values
    ...
    if year==2024:
            ...
            for i in range(max_iter):
                ...
                if (evaluate(x)):
                    ...
                    if new_profit > max_profit:
                        ...
                        pd.DataFrame(x).to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_解_1.csv", encoding='gbk',
                                               index=False)
                        max_profit = new_profit
                    if new_profit>profit:
                        profit=new_profit
                        pd.DataFrame(x).to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_解_1_{j}年解.csv", encoding='gbk',
                                               index=False)
            with open(f"preprocessed_data/整理数据/Q1/{year}/{year}_利润_1_{j}.txt", 'w', encoding='utf-8') as file:
        ...
        for i in range(max_iter):
            ...
            if(evaluate(x)):
                ...
                if new_profit>max_profit:
                    print(year,":","new_profit",new_profit)
                    pd.DataFrame(x).to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_解_1.csv",encoding='gbk',index=False)
                    max_profit=new_profit
            profit_history.append(max_profit)
        plt.plot(range(max_iter), profit_history)
        plt.xlabel('Iteration')
        plt.ylabel('Max Profit')
        plt.savefig(f"preprocessed_data/整理数据/Q1/{year}/{year}迭代图")
        plt.close()


    with open(f"preprocessed_data/整理数据/Q1/{year}/{year}_利润_1.txt", 'w', encoding='utf-8') as file:
        file.write(str(max_profit))
        file.write('\n')
        file.write(str(pass_num))
    jie=pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_解_1.csv",encoding='gbk')
    ans=pd.read_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_1.csv",encoding='gbk')
    ans.iloc[:, 1:42] = jie.values
    ans.to_csv(f"preprocessed_data/整理数据/Q1/{year}/{year}_答案表_1.csv",encoding='gbk',index=False)
