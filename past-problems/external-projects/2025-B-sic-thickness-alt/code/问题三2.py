...
# 衬底折射率计算函数
def n_substrate(lambda_nm, polarization):
    """
    计算衬底对特定偏振光的折射率
    lambda_nm: 波长（纳米）
    ...
    if polarization == 's':
        ...
        A, B = 5.59, 0.029
    return math.sqrt(1 + A * lambda_um**2 / (lambda_um**2 - B))
...
def calc_amplitudes(n2, lambda_nm):
    ...
    lambda_nm: 波长（纳米）
    """
    ns3 = n_substrate(lambda_nm, 's')
    np3 = n_substrate(lambda_nm, 'p')
    ...
    rs23 = (sqrt_expr - math.sqrt(ns3**2 - n1**2 * sin_theta1**2)) / (sqrt_expr + math.sqrt(ns3**2 - n1**2 * sin_theta1**2))
    rp23 = (math.sqrt(np3**2 - n1**2 * sin_theta1**2) - sqrt_expr) / (math.sqrt(np3**2 - n1**2 * sin_theta1**2) + sqrt_expr)
...
def calculate_d_for_data(nu1, r1, nu2):
    ...
    # 使用第一组数据求解n2
    # 初始猜测值（碳化硅折射率通常在2.5-3.0之间）
    n2_initial_guess = 2.6
...
def main():
    ...
    data_groups = [
        # 格式: (nu1, r1, nu2)
        (1860.270965,0.18330495, 2096.36614),  # 第一组数据
        (2096.36614, 0.1875981, 2365.041704),   # 第二组数据
        (2365.041704, 0.1860361, 2610.110724),    # 第三组数据
        (2610.110724,0.19142002, 2869.73334),    # 第四组数据
        (2869.73334, 0.19194731, 3122.50014076)     # 第五组数据
    ]
