...
def find_best_week_for_group(df_group, success_threshold=0.95, min_window_samples=10, max_window_samples=50):
    """
    为数据子集找到最佳孕周。
    【核心修改】窗口宽度修改为2周。
    """
    ...
    possible_weeks = range(10, 27)  # 结束周改为27，确保 week+2 不会超出范围
    valid_weeks_with_stats = []

    for week in possible_weeks:
        # 【核心修改】窗口宽度从 week+1 变为 week+2
        week_window_df = df_group[(df_group['孕周'] >= week) & (df_group['孕周'] < week + 2)]

        if min_window_samples <= len(week_window_df) < max_window_samples:
    ...
    # 【核心修改】窗口宽度从 week+1 变为 week+2
    final_window_df = df_group[(df_group['孕周'] >= best_week) & (df_group['孕周'] < best_week + 2)]
...
def objective_function(boundaries, df, w_risk, w_accuracy, min_window_samples=10, max_window_samples=50):
    """目标函数。"""
    ...
    for i in range(len(boundaries) - 1):
        ...
        best_week, _, risk_value = find_best_week_for_group(df_group,
...
def plot_results(df, group_results):
    ...
    for result in group_results:
        ...
        rect = Rectangle((bmi_min, optimal_week), bmi_max - bmi_min, 2, linewidth=2.5, edgecolor='limegreen', facecolor='limegreen', alpha=0.4,
                         linestyle='--')
        ax.add_patch(rect)
        # 【核心修改】文本内容显示为 week 到 week+2
        text_content = f"推荐孕周: {optimal_week}-{optimal_week + 2}\n窗口内成功率: {result['accuracy']:.2%}"
        plt.text((bmi_min + bmi_max) / 2, optimal_week + 2.2, text_content, ha='center', va='bottom', fontsize=10, fontweight='bold',
                 color='darkgreen')
...
def plot_error_analysis_grouped(analysis_results):
    ...
    # 【核心修改】调整了图形尺寸，使其不那么“长”
    fig, ax = plt.subplots(figsize=(14, 8))
    rects1 = ax.bar(x - width / 2, baseline_rates, width, label='组内基线成功率 (该BMI分组内所有样本)', color='gray')
    rects2 = ax.bar(x + width / 2, optimized_rates, width, label='优化后成功率 (仅推荐2周窗口内样本)', color='limegreen')
    ...
    # 【核心修改】设置Y轴的起始点，以“放大”顶部的趋势差异
    # 计算所有数据中的最小值，并以此为基础设置下限，留出一些边距
    min_rate = min(min(baseline_rates), min(optimized_rates))
    ...
    def autolabel(rects, color='black'):
        for rect in rects:
            ...
            ax.annotate(f'{height:.2%}',
                        ...
                        fontsize=14)  # 【核心修改】显著增大条形图上的数值字体
...
def main():
    ...
    min_window_samples_for_opt = 50
    max_window_samples_for_opt = 100
    print(f"窗口样本数约束设置为: {min_window_samples_for_opt} <= N < {max_window_samples_for_opt}")

    result = minimize(
        fun=objective_function,
        x0=initial_boundaries,
        args=(df, 0.7, 0.3, min_window_samples_for_opt, max_window_samples_for_opt),
        method='SLSQP',
        bounds=bounds,
    ...
    # --- 2. 进行组内对比的误差分析 ---
    print("\n" + "=" * 80)
    print("--- 2. 检测误差分析：组内基线 vs 优化后窗口 (2周窗口) ---")
    print("=" * 80)
    ...
    for i in range(len(optimal_boundaries) - 1):
        ...
        best_week, accuracy_in_window, _ = find_best_week_for_group(df_group,
                                                                    min_window_samples=min_window_samples_for_opt,
                                                                    max_window_samples=max_window_samples_for_opt)
        window_count = 0
        ...
        if best_week != -1:
            # 【核心修改】文本内容显示为 week 到 week+2
            print(f"推荐窗口: {best_week}-{best_week + 2} 周 (共 {window_count} 个样本)")
