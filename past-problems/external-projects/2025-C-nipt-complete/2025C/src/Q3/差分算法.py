...
def objective_function(boundaries, df, w_risk, w_accuracy, min_window_samples=10, max_window_samples=50):
    ...
    for i in range(num_groups):
        ...
        group_bmi_midpoint = (boundaries[i] + boundaries[i + 1]) / 2
        best_week, _, risk_value, is_strict_pass = find_best_week_for_group(
            df_group, group_bmi_midpoint, min_window_samples, max_window_samples
        )
    ...
    return w_risk * avg_risk + w_accuracy * (1 - total_accuracy) + penalty
...
def plot_error_analysis_grouped(analysis_results):
    ...
    rects2 = ax.bar(x + width / 2, optimized_rates, width, label='优化后成功率', color='limegreen')
...
def main():
    ...
    print(f"目标函数评估次数: {result.nfev}")
    print(f"最终目标函数值 (加权风险+不准确率): {result.fun:.4f}")
    print(f"最优决策变量 (BMI分割点): {[round(b, 2) for b in optimal_boundaries]}")
    print("=" * 80 + "\n")
    ...
    for i in range(len(optimal_boundaries) - 1):
        ...
        best_week, accuracy_in_window, _, _ = find_best_week_for_group(
            df_group, group_bmi_midpoint, min_window_samples_for_opt, max_window_samples_for_opt
        )
    ...
    print(f"{'BMI分组':<10} | {'BMI区间':<22} | {'总样本数':<10} | {'最佳NIPT时点 (周)':<18} | {'窗口样本数':<12} | {'窗口内成功率':<15}")
    ...
    for i, res in enumerate(group_results_for_plot):
        ...
        print(
            f"分组 {i + 1:<8} | {bmi_range_str:<22} | {total_samples:<10} | {optimal_week_str:<18} | {res['window_count']:<12} | {f'{res["accuracy"]:.2%}':<15}")
