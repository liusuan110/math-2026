...
# 定义参数
cases = [
    {'p1': 0.1, 'Cb1': 8, 'Cd1': 1, 'p2': 0.1, 'Cb2': 12, 'Cd2': 2, 'pf': 0.1, 'Ca': 8, 'Cdf': 4, 'Ct': 6}
]
...
for D1, D2, Df, Dd, Dr1, Dr2, Dr, Dc1, Dc2 in itertools.product([0, 1], repeat=9):
    for case in cases:
        ...
        C_det = (n * (D1 * case['Cd1'] + D2 * case['Cd2']) + N1 * Df * case['Cdf'] +
                 (case['pf'] * n * (D1*(1-case['p1']) + D2*(1-case['p1'])) + n * (1 - PF)*((1-D1)*(1-D2))) *
                 (Df * Dd * (case['Cd1']*Dc1*(1-D1) + case['Cd2']*Dc2*(1-D2))))

        C_ass = N1 * case['Ca'] + N*(D1*(1-case['p1'])+D2*(1-case['p2'])) * (
            Df*Dd*case['pf']*case['Ca']/2*(2-Dc1-Dc2+Dc1*(1-case['p1'])*(1-D1)+Dc2*(1-case['p2'])*(1-D2))+
            (1-Df)*case['pf']*case['Ca']/2*(2+(1-case['p1'])*(1-D1)+(1-case['p2'])*(1-D2))+
            N*(2-D1-D2)*(
                Df*Dd*(1-PF)*case['Ca']/2*(2-Dc1-Dc2+Dc1*(1-case['p1'])*(1-D1)+Dc2*(1-case['p2'])*(1-D2))+
                (1-Df)*case['Ca']/2*(1-PF)*(2+(1-case['p1'])*(1-D1)+(1-case['p2'])*(1-D2))
            )
        )
        C_dis = N * case['Ct'] * (Dd) * case['pf'] * (D1*(1-case['p1']) + D2 * (1-case['p2'])) + (1 - PF) * N * case['Ct'] * (Dd) * ((1-D1)+(1-D2))
