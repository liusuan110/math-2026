...
for year in range(2024,2031):
    ...
    ans_total_sheet.to_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年答案表.csv",encoding='gbk',index=False)
    shengchangqingkuang=pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年生产销售表.csv",encoding='gbk')
    yuqixiaoshou=pd.read_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年农作物预期销量表.csv",encoding='gbk')
    ...
    pivot_df.to_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年_销量表.csv",encoding='gbk',index=False)
    ...
    danjia_df.to_csv(f"preprocessed_data/整理数据/Q2/{year}/{year}年_单价表.csv", encoding='gbk', index=False)


    #成本表
    df2=pd.read_csv("preprocessed_data/整理数据/答案总表模板.csv",encoding='gbk')
    df2.fillna(0,inplace=True)
    df1=pd.read_csv(f'preprocessed_data/整理数据/Q2/{year}/{year}年生产销售表.csv',encoding='gbk')
    ...
    df2.to_csv(f'preprocessed_data/整理数据/Q2/{year}/{year}年_成本表.csv', encoding='gbk', index=False)
    ...
    # 保存更新后的 df2
    df2.to_csv(f'preprocessed_data/整理数据/Q2/{year}/{year}年_产量表.csv', encoding='gbk', index=False)